...
expr = (-1)**n *  (-sp.sin(sp.pi * m / 2) * sp.sin(sp.pi * phi / 2) + sp.cos(sp.pi * m / 2) * sp.cos(sp.pi * phi / 2)) * sp.sinc((m/2)) *sp.DiracDelta(u-(N/(k*delta)))
expr_envalope=a *(sp.sinc(sp.pi*a*N/k))
# Compute the summation for each solution of n

result_sub=expr.subs({k:4,N:1,m:1-4*n}) #!!!! m=N-km, change N , k and m at the same time.
...
result_en_sub=expr_envalope.subs({k:4,N:1})
...
envalope=sp.simplify(result_en_sub)
print("square of sinc envalope=")

sp.pprint(envalope**2)
